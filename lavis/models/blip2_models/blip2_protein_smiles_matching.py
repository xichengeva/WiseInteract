...
class Blip2ITMCPI(Blip2QformerCPI):
    ...
    def forward(self, samples, match_head="psm", test_path = ''):
        ...
        if match_head == "pcm":
        ...
        elif match_head == "itc_heatmap":
            query_tokens = self.query_tokens.expand(proteins_embeds.shape[0], -1, -1) # torch.Size([100, 32, 768])
            query_output = self.Qformer.bert(query_embeds=query_tokens, encoder_hidden_states=proteins_embeds, encoder_attention_mask=proteins_atts, use_cache=True, return_dict=True)
            protein_feats = F.normalize(self.protein_proj(query_output.last_hidden_state), dim=-1)
            
            text_output = self.Qformer.bert(t_smiles.input_ids,attention_mask=t_smiles.attention_mask,return_dict=True)
            ...
            sim_q2t = torch.matmul(protein_feats_all.unsqueeze(1), text_feat_all.unsqueeze(-1)).squeeze()# [batch_size, batch_size*num_gpu, num_query_tokens]
            sim_i2t, _ = sim_q2t.max(-1)
            prePath = '/p2s/'
            plot_heatmap(sim_i2t.cpu(), samples['batches'][0], prePath)
            sims = torch.bmm(protein_feats, text_feat.unsqueeze(-1))
            return sims
        
        elif match_head == "pcc_pca":
            text_feat = t_smiles['input_ids'].cpu()
            protein_feats = proteins_embeds.cpu() 
            f = h5py.File('/tensorProBefore1/'+ test_path +'.h5', 'a')
            f.create_dataset(f'tensor_{self.index}', data = protein_feats.numpy(), chunks=True)
            self.index = self.index + 1

            query_tokens = self.query_tokens.expand(proteins_embeds.shape[0], -1, -1) # torch.Size([100, 32, 768])
            query_output = self.Qformer.bert(query_embeds=query_tokens, encoder_hidden_states=proteins_embeds, encoder_attention_mask=proteins_atts, use_cache=True, return_dict=True)
            protein_feats = F.normalize(self.protein_proj(query_output.last_hidden_state), dim=-1) # torch.Size([bs, 32, 256])
            
            text_output = self.Qformer.bert(t_smiles.input_ids,attention_mask=t_smiles.attention_mask,return_dict=True)
            text_feat = F.normalize(self.text_proj(text_output.last_hidden_state[:, 0, :]), dim=-1) # torch.Size([bs, 256])

            sims = torch.bmm(protein_feats, text_feat.unsqueeze(-1)) # torch.Size([100, 32, 1])
            sim, _ = torch.max(sims, dim=1)
            return sim
        
        elif match_head == "pcc_pcm":
            query_tokens = self.query_tokens.expand(proteins_embeds.shape[0], -1, -1) # torch.Size([100, 32, 768])
            query_output = self.Qformer.bert(query_embeds=query_tokens, encoder_hidden_states=proteins_embeds, encoder_attention_mask=proteins_atts, use_cache=True, return_dict=True)
            protein_feats = F.normalize(self.protein_proj(query_output.last_hidden_state), dim=-1) # torch.Size([736, 32, 256])
            
            text_output = self.Qformer.bert(t_smiles.input_ids,attention_mask=t_smiles.attention_mask,return_dict=True)
            text_feat = F.normalize(self.text_proj(text_output.last_hidden_state[:, 0, :]), dim=-1) # torch.Size([736, 256])
