...
class Blip2ITMCPI(Blip2QformerCPI):
    ...
    def forward(self, samples, match_head="psm", test_path = ''):
        ...
        if match_head == "pcm":
            ...
            output_pcm = self.Qformer.bert(t_smiles.input_ids, query_embeds=query_tokens, attention_mask=attention_mask, encoder_hidden_states=proteins_embeds, encoder_attention_mask=proteins_atts, return_dict=True)
            pcm_embeddings = output_pcm.last_hidden_state[:, : query_tokens.size(1), :]
            pcm_logit = self.pcm_head(pcm_embeddings)
            pcm_logit = pcm_logit.mean(dim=1) 
            pcm_logit = pcm_logit[:, 1] 
            return pcm_logit
            ...
            sims = torch.bmm(proteins_feats, t_smiles_feat.unsqueeze(-1)) 
